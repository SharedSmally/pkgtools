...
sub genFriendDefCode{
	...
	if ($v0) {
        if ($node->hasAttribute("cfunc") || $node->hasAttribute("func")) {
    	    if (isXmlTrue($node, "cfunc") || isXmlFalse($node, "func")) {
       			$x0 = "friend extern \"C\" {"; push(@ha, $x0);
       			$x0 = "${SP1}${v0};"; push(@ha, $x0);
       			$x0 = "}"; push(@ha, $x0);
        	} else {    	    	
...
sub genUsingCode {
    ...
    my $ns="namespace"; $ns="" if (isXmlFalse($node,"namespace"));
...
sub genDeclCode {
    ...
    my @ha; my ($x0,$x1,$t1,$v0);
    my $ns=getXmlAttr($node,"namespace",""); $ns .="::" if ($ns);
    my $p=""; $p="friend " if (isXmlTrue($node, "friend"));
    ...
    $category="${p}class " if (isXmlTrue($node,"isclass"));
...
sub getFuncDeclStrs {
    ...
    if ($ret){
        ...
        $ret="static $ret" if (isXmlTrue($node,"static"));
    ...
    my $const=""; $const=" const" if (isXmlTrue($node,"const"));
...
sub getVarPrefix {
   ...
   my $psa=$sp; $psa .= "static " if (isXmlTrue($node,"static"));
   $psa .= "const " if (isXmlTrue($node,"const") || ($node->nodeName eq "const")) ;
   
   my $ref=" ";  $ref = " & " if (isXmlTrue($node,"ref")) ;
...
sub genVarCode {    
    ...
    my $init= isXmlTrue($node,"const") || ($node eq "const") || isXmlTrue($node,"static");
    #return ("$sa = $x0;", "") if (isInteger($t0) && $init );  #integer

    $x0= "\"$x0\"" if (isXmlTrue($node,"string"));
    return ("$sa = $x0;", "") if (isXmlTrue($node,"inline"));  #inline
    
    my $sc="$sap$t0${ref}$n0=$x0;";

    if (isXmlTrue($node,"init")) { return ($sc,""); }
    else { return ($sa.";", $sc ); }  
}
