...
use CommonXml qw( xmlRoot getXmlAttr xml_true xml_false);
...
sub getTypeStr {
  ...
  $cls="typename ${cls}" if (xml_true($node, "typename"));
...
sub getArgStrs {
  ...
  $df="\"$df\"" if (xml_true($node, "string") && ($df || $node->hasAttribute("default")) );
...
sub getArgTypeStr {
   ...
   my $t1=not(xml_false($node,"ref"));
   
   if (isSimpleType($ret) || ($ret =~ m/\&$/) || ($ret =~ m/\*$/) ) { 
      $ret .= " & " if (xml_true($node,"ref")); #ref if explicitly true
   } else { $ret .= " & " if (not xml_false($node,"ref")); } # ref if not explicit false 
   
   $ret = "const $ret" if (xml_true($node,"const"));
...
############################################################################
sub isInline {
    return 1 if (xml_true($_[0],"inline"));
    return 0 if (xml_false($_[0],"inline"));
